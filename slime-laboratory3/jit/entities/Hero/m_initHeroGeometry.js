...
/*
	Index: 1139
	Path:  entities/Hero::m_initHeroGeometry
	Type:  Protected
	Kind:  Method
...
//                        stack1 = this;// JIT: redundant assigment, value unused
                        ...
                        // 7 0::angularDamping
                        if (!(local4 != undefined && local4[AX_CLASS_SYMBOL])){
                            local4.angularDamping = stack1;
                        } else {
                            context.setproperty($names[33], stack1, local4);
                        }
                        { p = 1205; continue; };
                    case 1197:
//                        stack0 = local4;// JIT: redundant assigment, value unused
//                        stack1 = 1000;// JIT: redundant assigment, value unused
                        // 7 0::angularDamping
                        if (!(local4 != undefined && local4[AX_CLASS_SYMBOL])){
                            local4.angularDamping = 1000;
                        } else {
                            context.setproperty($names[33], 1000, local4);
                        }
                    case 1205:
                        // 7 0:Box2D.Collision.Shapes::b2CircleShape
                        stack0 = scope0.findScopeProperty($names[3], true, false);
...
//                        stack1 = this;// JIT: redundant assigment, value unused
                        ...
                        /* We sure that this safe get, represented in TRAIT as Slot, type: 7 0::int  */ 
                        stack1 = this.$BgnumCircles;
                        if (stack1 > local10) { p = 1066; continue; };
                        // 7 0:Box2D.Dynamics.Joints::b2DistanceJointDef
                        stack0 = scope0.findScopeProperty($names[34], true, false);
                        //JIT: Support fast construct:b2DistanceJointDef/b2JointDef/Object
                        stack0 = context.constructFast(stack0, [], $names[34]);
...
//                        stack2 = 2;// JIT: redundant assigment, value unused
                        stack1 /= 2;
                        if (stack1 > local10) { p = 1544; continue; };
                        // 7 0:Box2D.Dynamics::b2BodyDef
                        stack0 = scope0.findScopeProperty($names[5], true, false);
                        //JIT: Support fast construct:b2BodyDef/Object
                        stack0 = context.constructFast(stack0, [], $names[5]);
...
//                        stack1 = 0.1;// JIT: redundant assigment, value unused
                        // 7 0::angularDamping
                        if (!(local4 != undefined && local4[AX_CLASS_SYMBOL])){
                            local4.angularDamping = 0.1;
                        } else {
                            context.setproperty($names[33], 0.1, local4);
                        }
//                        stack0 = local4;// JIT: redundant assigment, value unused
                        // 7 0:Box2D.Dynamics::b2Body
                        temp = scope0.findScopeProperty($names[6], true, false);
...
//                        stack1 = 0.2;// JIT: redundant assigment, value unused
                        ...
                        if (!(local8 != undefined && local8[AX_CLASS_SYMBOL])){
                            ...
                            context.setproperty($names[16], 0.2, local8);
                        }
//                        stack0 = local8;// JIT: redundant assigment, value unused
//                        stack1 = 0.1;// JIT: redundant assigment, value unused
                        // 7 0::density
                        if (!(local8 != undefined && local8[AX_CLASS_SYMBOL])){
                            ...
                            context.setproperty($names[15], 0.1, local8);
                        }
//                        stack0 = local8;// JIT: redundant assigment, value unused
//                        stack1 = 0.0001;// JIT: redundant assigment, value unused
                        // 7 0::restitution
                        if (!(local8 != undefined && local8[AX_CLASS_SYMBOL])){
                            local8.restitution = 0.0001;
                        } else {
                            context.setproperty($names[19], 0.0001, local8);
                        }
//                        stack0 = this;// JIT: redundant assigment, value unused
                        // 7 0::Utils
                        temp = scope0.findScopeProperty($names[10], true, false);
...
//                        stack2 = local8;// JIT: redundant assigment, value unused
                        if (!(_a = typeof stack1, ((_a !== 'object' && _a !== 'function' ) || stack1 == null || stack1[AX_CLASS_SYMBOL]))) {
                           stack1 = stack1.CreateFixture(local8);
                        } else {
                            // 7 0::CreateFixture
                            {
                                let t = stack1;
                                const m = t.$BgCreateFixture || (t = sec.box(stack1), t.$BgCreateFixture);
                                if( typeof m === "function" ) { 
                                    stack1 = t.$BgCreateFixture (local8);
                                } else { 
                                    stack1 = stack1.axCallProperty($names[25], [local8], false);
                                }
                            }
                        }
                        // 7 0::heroFrostFixture
                        /* We sure that this safe set, represented in TRAIT as Slot, with type: 7 0:Box2D.Dynamics::b2Fixture  */ 
                        this.$BgheroFrostFixture = stack1;
//                        stack0 = this;// JIT: redundant assigment, value unused
                        // 7 0::heroFrostBody
                        //Possible type:7 0:Box2D.Dynamics::b2Body
                        /* We sure that this safe get, represented in TRAIT as Slot, type: 7 0:Box2D.Dynamics::b2Body  */ 
                        ...
                        if (!(_a = typeof stack0, ((_a !== 'object' && _a !== 'function' ) || stack0 == null || stack0[AX_CLASS_SYMBOL]))) {
                            ...
                            // 7 0::SetActive
                            {
                                ...
                                if( typeof m === "function" ) { 
                                   ...
                                   stack0.axCallProperty($names[46], [false], false);
                                }
                            }
                        }
                        return;
...
//# sourceURL=http://jit/entities/Hero/m_initHeroGeometry.js
})